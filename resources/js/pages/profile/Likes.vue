...
<template>
    <AppLayout>
        <Head :title="`${profile.name} — ${t('profile.tabs.likes')}`" />
        <ProfileHeader :profile="profile" active-tab="likes" />

        <EmptyState v-if="!titles.data.length" :icon="Heart" :title="t('profile.emptyLikes')" />
