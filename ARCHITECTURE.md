...
- **Case-sensitivity del filesystem** (CI en Linux vs. desarrollo en Windows): el default de `inertia-laravel` para localizar componentes en testing es `resource_path('js/Pages')` con «P» mayúscula, pero el starter kit de Vue usa `js/pages` en minúscula. En Windows y macOS la ruta resuelve igual porque el filesystem es case-insensitive; en Linux no existe y **todos los tests con `assertInertia` fallan** con `Inertia page component file [X] does not exist`. Por eso `config/inertia.php` está publicado en el repo con el case real fijado explícitamente en `page_paths` y `testing.page_paths`. La misma clase de error puede aparecer en cualquier ruta de archivo (vistas Blade, assets, imports del frontend) — no en nombres de clase PHP, que son case-insensitive.
